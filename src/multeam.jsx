      ...
      {(() => {
        ...
        return (
          <div style={{ marginBottom:20 }}>
            <p style={{ margin:"0 0 12px", fontSize:12, fontWeight:700, color:T.sub, textTransform:"uppercase", letterSpacing:1 }}>🏆 Ranking</p>
                ...
                {podiumItems.map(({ m, place, medal, h, sz }) => {
                  ...
                  return (
                    <div key={m.id} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                      ...
                      <div style={{ background:m.paid>0?(isFirst?T.green:`${T.green}22`):(m.unpaid>0?(isFirst?T.brand:`${T.brand}22`):T.bg), borderRadius:10, padding:"4px 10px" }}>
                        <p style={{ margin:0, fontWeight:900, fontSize:isFirst?18:14, color:m.unpaid>0?(isFirst?"#fff":T.brand):(isFirst?T.green:T.sub) }}>
                          {m.paid>0?`${m.paid}€ pagos`:(m.unpaid>0?`${m.unpaid}€ deve`:"✓ OK")}
                        </p>
                      </div>
